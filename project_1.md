...
Project 1should be done in groups of two or individually.
...
_Note_: Most webservers nowadays require TLS encryption (HTTP**S**) on the connection.  That's a good thing, but it would add a lot of complexity to this assignment.  We are testing with "insecure.stevetarzia.com" because that is a webserver that I configured specially to **not** require encryption.
...
*   Your client should understand and follow 301 and 302 redirects.  When you get a redirect, your client should make another request to fetch the corrected url.  Your client should also print a message to **stderr** with the format: "Redirected to: [http://other.com/blah](http://other.com/blah%22)"  _(The specific url should be shown)_.
    ...
    *   An example of a url with a 302 temporary redirect is:  
        [http://maps.google.com/](http://maps.google.com/) (which redirects to [http://maps.google.com/maps](http://maps.google.com/maps,%20), which redirects to [https://www.google.com:443/maps](https://www.google.com/maps))
    *   Another example is:  
...
*   Name your source files "http\_client.py", "http\_server1.py", etc.
...
*   Your submission should be a .tgz file (a gzipped tarball), including a README.txt, and the python sources.  To create the archive, use a command like the following
    *   tar -czvf project1\_NETID1\_NETID2.tgz README.txt \*.py
